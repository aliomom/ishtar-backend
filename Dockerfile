...
RUN apt update -y && apt install zip libicu-dev  -y && curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer && docker-php-ext-install intl && composer update && composer require symfony/translation && composer require doctrine/annotations && composer require symfony/orm-pack
